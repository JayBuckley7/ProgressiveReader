...
def get_books():
    ...
    try:
        # Query Firestore for books belonging to this user
        books_ref = fs_db.collection('books')
        query = books_ref.where('userId', '==', user_id).order_by('uploadedAt', direction='DESCENDING')
        docs = query.stream()
        ...
        for doc in docs:
            ...
            if 'uploadedAt' in book_data and hasattr(book_data['uploadedAt'], 'isoformat'):
                book_data['uploadedAt'] = book_data['uploadedAt'].isoformat()
            if 'lastOpened' in book_data and hasattr(book_data['lastOpened'], 'isoformat'):
                book_data['lastOpened'] = book_data['lastOpened'].isoformat()
...
def get_book(book_id):
    ...
    try:
        ...
        book_ref = fs_db.collection('books').document(book_id)
        ...
        if book_data.get('userId') != user_id:
...
def create_book():
    ...
    try:
        ...
        book_data = {
            ...
            'fileType': data.get('fileType', 'epub'),
            'userId': user_id,
        ...
        if data.get('id'):
            # Use provided ID
            book_ref = fs_db.collection('books').document(data['id'])
            book_ref.set(book_data)
            book_id = data['id']
        else:
            # Generate new ID
            book_ref = fs_db.collection('books').document()
            book_ref.set(book_data)
            book_id = book_ref.id
...
def update_book(book_id):
    ...
    try:
        ...
        updates = {}
        allowed_fields = ['title', 'coverUrl', 'lastOpened', 'totalChapters']
        ...
        updated_book = book_ref.get().to_dict()
        updated_book['id'] = book_id
        
        # Convert timestamps
        if 'uploadedAt' in updated_book and hasattr(updated_book['uploadedAt'], 'isoformat'):
            updated_book['uploadedAt'] = updated_book['uploadedAt'].isoformat()
        if 'lastOpened' in updated_book and hasattr(updated_book['lastOpened'], 'isoformat'):
            updated_book['lastOpened'] = updated_book['lastOpened'].isoformat()
...
@api_bp.route('/books/<book_id>/download', methods=['GET'])
@require_auth
...
@api_bp.route('/books/upload', methods=['POST'])
@require_auth
def upload_book():
    ...
    try:
        ...
        book_data = {
            ...
            'fileType': request.form.get('fileType', file_ext[1:]),  # Remove the dot
            'userId': user_id,
            'uploadedAt': datetime.utcnow(),
            'totalChapters': int(request.form.get('totalChapters', 0)) if request.form.get('totalChapters') else None,
            'coverUrl': request.form.get('coverUrl'),
            'filePath': file_path  # Store local file path
        }
...
def save_progress():
    ...
    try:
        progress_data = {
            ...
            'currentPosition': data.get('currentPosition', 0),
            'lastUpdated': datetime.utcnow()
        }
        
        # Use bookId and userId as composite key
        progress_id = f"{user_id}_{data.get('bookId')}"
        progress_ref = fs_db.collection('progress').document(progress_id)
...
@api_bp.route('/progress/<book_id>', methods=['GET'])
@require_auth
def get_progress(book_id):
    ...
    try:
        progress_id = f"{user_id}_{book_id}"
        progress_ref = fs_db.collection('progress').document(progress_id)
        ...
        if 'lastUpdated' in progress_data and hasattr(progress_data['lastUpdated'], 'isoformat'):
            progress_data['lastUpdated'] = progress_data['lastUpdated'].isoformat()
...
def get_settings():
    ...
    try:
        settings_ref = fs_db.collection('settings').document(user_id)
...
def save_settings():
    ...
    try:
        ...
        settings_ref = fs_db.collection('settings').document(user_id)
        settings_ref.set(data, merge=True)  # Merge to preserve existing fields
