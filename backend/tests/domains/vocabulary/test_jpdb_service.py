...
from app.domains.vocabulary.integrations import JpdbProvider, JpdbApiProvider
...
def _config() -> Dict[str, Any]:
    return {
        ...
        'JPDB_VOCAB_FIELDS': [
            'vid', 'sid', 'rid', 'spelling', 'reading', 'frequency_rank', 'part_of_speech',
            'meanings_chunks', 'meanings_part_of_speech', 'card_state', 'pitch_accent'
        ],
        'JPDB_API_URL': 'http://example.test',
    }


def test_get_jpdb_data_empty_returns_empty():
    svc = VocabularyService(_MockModuleProvider(), _MockApiProvider())
