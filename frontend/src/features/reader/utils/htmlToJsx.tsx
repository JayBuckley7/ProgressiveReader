...
export type HighlightFn = (text: string) => React.ReactElement[];
...
export function parseHtmlToJsx(html: string, highlightFn?: HighlightFn): React.ReactElement {
  ...
  const convertNode = (node: Node, key: number): React.ReactElement | string | null => {
    if (node.type === "text") {
    ...
    if (node.type === "tag" || node.type === "script" || node.type === "style") {
      ...
      const children = (el.children || [])
        .map((child: Node, i: number) => convertNode(child, i))
        .filter(Boolean);
