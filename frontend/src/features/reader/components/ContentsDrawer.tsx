...
export default function ContentsDrawer({
  ...
  const handleBookmarkSelection = (bookmark: Bookmark) => {
    if (onSelectBookmark) {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute left-0 top-0 h-full w-[calc(100vw-1rem)] max-w-sm bg-white dark:bg-gray-800 shadow-xl transform transition-transform ${visible ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="flex" role="tablist" aria-label={t('reader.toc.menu')}>
          <button type="button" role="tab" aria-selected={activeTab === "toc"} className={tabClass("toc")} onClick={() => setActiveTab("toc")}>{t('reader.toc.tab.toc')}</button>
          <button type="button" role="tab" aria-selected={activeTab === "bookmarks"} className={tabClass("bookmarks")} onClick={() => setActiveTab("bookmarks")}>{t('reader.toc.tab.bookmarks')}</button>
        </div>
            ...
            <ul className="space-y-1">
              {chapterTitles.map(ch => (
                <li key={ch.index}>
                  <button
                    type="button"
                    onClick={() => { onSelectChapter(ch.index); onClose(); }}
                    aria-current={ch.index === currentChapter ? "location" : undefined}
                    className={`block min-h-11 w-full rounded px-3 py-2 text-left text-sm transition-colors ${ch.index === currentChapter ? "bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
                  >
                    {ch.title || t('reader.chapterNumber', { number: ch.index + 1 })}
                  </button>
                </li>
              ))}
            </ul>
              ...
              {bookmarks && bookmarks.length > 0 ? (
                <ul className="space-y-2">
                  {bookmarks.map((bookmark) => (
                    <li key={String(bookmark.id ?? `${bookmark.bookId}:${bookmark.chapterIndex}:${bookmark.position}`)}>
                      <button
                        type="button"
                        onClick={() => handleBookmarkSelection(bookmark)}
                        className="min-h-11 w-full rounded px-3 py-2 text-left transition-colors hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:hover:bg-gray-700"
                      >
                        <span className="block text-sm font-medium">
                          {chapterTitles?.[bookmark.chapterIndex]?.title ||
                            t("reader.chapterNumber", { number: bookmark.chapterIndex + 1 })}
                        </span>
                        {typeof bookmark.note === "string" && bookmark.note.trim() ? (
                          <span className="block text-xs text-gray-600 dark:text-gray-400">{bookmark.note}</span>
                        ) : null}
                      </button>
                    </li>
                  ))}
