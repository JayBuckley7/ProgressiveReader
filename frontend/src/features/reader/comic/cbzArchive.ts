...
import { parseComicInfo } from '@features/books/comics/comicInfo';
...
export async function openCbz(data: ArrayBuffer) {
  ...
  try {
    ...
    const metadata = entries.find(entry => !entry.directory && entry.filename.toLowerCase() === 'comicinfo.xml');
    let comicInfo = null;
    if (metadata?.getData && metadata.uncompressedSize <= 262144) {
      try { const blob = await metadata.getData(new BlobWriter(), { onprogress: async count => { if (count > 262144) throw new Error('Comic metadata exceeds limit'); } }); comicInfo = parseComicInfo(await blob.text(), pages.length); } catch { /* Unreadable metadata must not prevent reading page images. */ }
    }
    return {
      comicInfo,
