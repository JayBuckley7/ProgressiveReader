...
export interface OCRProgress {
  ...
  total?: number;
  percent?: number;
  pdf?: string; // base64 encoded PDF when complete
  filename?: string;
  error?: string;
  // Some backends include extra fields like `size`; keep it open.
  [key: string]: unknown;
}
...
/**
 * Process PDF with OCR and return the OCR'd PDF.
 * Progress updates are provided via the optional callback.
 */
export async function processPDFWithOCR(file: File, onProgress?: OCRProgressCallback): Promise<File> {
          ...
          if (line.startsWith("data: ")) {
            ...
            try {
              ...
              if (data.type === "complete") {
                ...
                const expectedSize = (data as any).size
                  ? ((data as any).size / (1024 * 1024)).toFixed(2) + "MB"
                  : "unknown";
