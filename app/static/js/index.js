...
async function initializeApp() {
    ...
    console.log(`${logPrefix} Initial check: window.dbKeyVal available:`, !!window.dbKeyVal, "window.dbKeyVal type:", typeof window.dbKeyVal);

    const recentBooksGrid = document.getElementById('recent-books-grid');
    // Get the container for all drive controls
    const driveControlsContainer = document.getElementById('drive-controls-container'); 
    ...
    try {
        ...
        console.log(`${logPrefix} After dbReady: window.dbKeyVal available:`, !!window.dbKeyVal, "window.dbKeyVal type:", typeof window.dbKeyVal);

        renderBookshelf(driveSync);
        setupUploadForm();

        let driveButton = null;
        if (driveControlsContainer) { // Pass the container to DriveButton
            driveButton = new DriveButton(driveControlsContainer, driveSync);
        } else {
            console.warn(`${logPrefix} Drive controls container (#drive-controls-container) not found. DriveButton not initialized.`);
        }
        ...
        if (recentBooksGrid) {
            recentBooksGrid.addEventListener('click', async (event) => {
                ...
                if (uploadButton) {
                    ...
                    if (!bookId || !bookTitle) {
                        console.error(`${logPrefix} Book ID or Title missing from upload button.`);
                        alert('Could not initiate upload: Book ID or Title missing.');
                        return;
                    }

                    console.log(`${logPrefix} Upload to Drive clicked for book ID: ${bookId}, Title: ${bookTitle}`);
                    ...
                    try {
                        ...
                        if (!bookData || !bookData.content) {
                            console.error(`${logPrefix} EPUB data (bookData.content) not found for book ${bookId}. BookData received:`, bookData);
                        ...
                        console.log(`${logPrefix} Calling driveSync.uploadBookToDrive with:`, { bookId, bookTitle, epubBlob: bookData.content });
                        
                        uploadButton.innerHTML = 'Uploading...';
                        
                        // Actual call to driveSync.uploadBookToDrive
                        const uploadedFile = await driveSync.uploadBookToDrive(bookId, bookTitle, bookData.content);

                        console.log(`[IndexInit] Upload successful for "${bookTitle}". Drive file ID: ${uploadedFile.id}`);
                        ...
                        console.error(`${logPrefix} Error during upload process for book ${bookId}:`, error);
                        alert(`Failed to upload "${bookTitle}": ${error.message || 'Unknown error'}`);
                        uploadButton.disabled = false;
        ...
        window.addEventListener('drive-sync-start', () => showBanner('Syncing…', 'syncing'));
        window.addEventListener('drive-sync-complete', () => showBanner('Up to date', 'idle', 4000));
        window.addEventListener('drive-offline', () => showBanner('Offline', 'offline'));
        ...
        try {
            ...
            if (driveSync.isConnected()) {
                console.log(`${logPrefix} Early driveSync.init() successful, Drive is connected.`);
            } else {
                console.log(`${logPrefix} Early driveSync.init() completed, but Drive is not connected.`);
            }
        } catch (err) {
