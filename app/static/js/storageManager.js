...
async function determineActualStartingPosition(bookId) {
    ...
    try {
        // Check localStorage for the latest saved progress
        ...
        if (localStorageProgressMeta && localStorageProgressMeta.index !== undefined) {
            ...
            latestTimestamp = localStorageProgressMeta.dttm_mod || 0;
            console.log(`[StorageManager] Loaded progress from localStorage for ${bookId}: page ${startIndex} (ts: ${latestTimestamp})`);
        }

    } catch (error) {
        console.error(`[StorageManager] Error determining starting position for ${bookId}:`, error);
        // Fallback to localStorage directly in case of an unexpected error
